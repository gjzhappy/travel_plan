...
class MockMapClient:
    def __init__(self, overrides: dict[tuple[str,str], int] | None = None): self.overrides=overrides or {}; self.calls=0
    def route(self, origin: Location, destination: Location, mode: str="public_transit") -> RouteResult:
        ...
        return RouteResult(duration,round(distance,2),mode,"mock_map",datetime.now(timezone.utc).isoformat())
