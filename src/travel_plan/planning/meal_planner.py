...
class MealPlanner:
    ...
    def insert(self,day,restaurants,req,hotel,only_meal=None):
        ...
        for meal,window in kinds:
            ...
            if meal=="lunch":
                before=[n for n in attractions if n.end_time<=window[1]]; previous=before[-1] if before else hotel
                after=[n for n in attractions if n.start_time>=window[0] and n is not previous]; next_node=after[0] if after else hotel
            else: previous=attractions[-1] if attractions else hotel;next_node=hotel
            ...
            for r in restaurants:
                ...
                candidates.append((pref-detour-r.price_per_person*.05,r,detour,a,b,cost))
            if not candidates:continue
            _,r,detour,leg,next_leg,cost=max(candidates,key=lambda x:x[0])
            ...
            node=Node(meal,r.name,start.strftime("%H:%M"),end.strftime("%H:%M"),None,req.transport,leg.distance_km,leg.duration_min,cost,{"cuisine":r.cuisine,"price_per_person":r.price_per_person,"detour_min":detour,"opening_hours":r.opening_hours,"lat":r.lat,"lon":r.lon,"next_travel_min":next_leg.duration_min,"previous_node":previous.name,"next_node":next_node.name})
